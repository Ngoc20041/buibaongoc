...
function generateRandomOrders(n: number, maxCapacity: number, maxTotalLoad: number): Order[] {
    ...
    for (let i = 0; i < n; i++) {
        const weight =
            Math.floor(Math.random() * (maxCapacity * 0.6)) + 1

        if (totalLoad + weight > maxTotalLoad) break
...
function generateSafeGasStationsByFuel(maxGap: number): GasStation[] {
    const stations: GasStation[] = []

    for (let x = Math.floor(maxGap / 2); x < COLS; x += maxGap) {
        for (let y = Math.floor(maxGap / 2); y < ROWS; y += maxGap) {
            stations.push({x, y})
        }
    }
...
export default function TestPage() {
    ...
    const MAX_TOTAL_LOAD = baseVehicle.capacity * 3
    const used = new Set<string>()
    ...
    orders.forEach(o => {
        if (o.weight > baseVehicle.capacity) {
            throw new Error(`Order ${o.id} vượt tải xe`)
        }
    ...
    const maxGap = Math.min(
        Math.floor(baseVehicle.maxFuel * 20 * 0.6),
        Math.floor(Math.min(COLS, ROWS) / 1.5)
    )


    let gasStations = generateSafeGasStationsByFuel(maxGap)

    if (gasStations.length === 0) {
        gasStations = generateGasStations(2, used)
    }
